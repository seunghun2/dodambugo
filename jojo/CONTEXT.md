...
- **배포**: Vercel (예정)
- **DB**: Supabase (기존 마음부고와 동일 DB, B2B 테이블만 추가)
- **결제**: INNOPAY
- **알림**: 솔라피 (카카오 알림톡 + SMS)
- **앱 패키징**: Capacitor (웹뷰 래핑)
- **인증**: JWT 토큰 + bcryptjs 비밀번호 해시
- **폰트**: Pretendard
...
## 🗄️ B2B 전용 DB 테이블 (SQL 작성 완료, Supabase 미생성)
...
1. 설정(마이페이지) 페이지 제작
2. Supabase에 B2B 테이블 실제 생성
3. 화환 결제 → 예치금 적립 연동 (핵심 BM)
4. B2B 관리자 페이지 (기존 /admin 확장)
5. Capacitor 앱 패키징
