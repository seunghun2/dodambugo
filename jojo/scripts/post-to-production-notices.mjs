...
async function postNoticesToProduction() {
  console.log('실서버 API(https://bugoon.maeumbugo.co.kr/api/b2b/admin/notices)에 공지 5건 POST 전송 시작...');

  for (const notice of officialNotices) {
    try {
      const res = await fetch('https://bugoon.maeumbugo.co.kr/api/b2b/admin/notices', {
        method: 'POST',
      ...
      if (res.ok && data.success) {
        console.log(`✓ 실서버 공지 등록 성공: ${notice.title}`);
      } else {
        console.error(`✕ 실서버 공지 등록 실패 (${notice.title}):`, data);
      }
    } catch (err) {
