...
const officialNotices = [
  ...
  },
  {
    ...
    content: `<p>안녕하세요. 부고온 파트너입니다.</p><p>파트너 회원 간 추천인 코드 혜택 및 상조소속 가입 시 적용 기준을 안내해 드립니다.</p><p>1. 추천인 코드 확인<br />- 메인 화면 및 내 정보에서 본인의 4자리 추천 코드를 확인하실 수 있습니다.</p><p>2. 추천 적립금 지급 기준<br />- 신규 파트너가 상조소속 없이(개인 파트너로) 가입 시 내 4자리 추천 코드를 입력하면 추천인(나)에게 2,500원 추천 적립금이 지급됩니다.</p><p>3. 상조회사 소속 가입 시 유의사항<br />- 신규 파트너가 가입 시 상조회사 소속을 선택하여 가입하는 경우, 해당 가입 건은 상조회사 본사 정산 라인으로 귀속되므로 개인 추천 적립금(2,500원)이 지급되지 않습니다.</p><p>파트너 여러분의 많은 이용 바랍니다.</p><p>감사합니다.</p>`,
    is_fixed: false,
  },
  {
...
async function seedNotices() {
  console.log('상조소속 가입 시 개인 추천금 미지급 기준 포함 공지 5종 DB 업데이트 시작...');
  ...
  if (error) {
    ...
    console.log(`🎉 성공적으로 [상조소속 추천금 미지급 기준 포함] 공지사항 ${data.length}건이 DB에 깔끔하게 등록되었습니다!`);
  }
}
