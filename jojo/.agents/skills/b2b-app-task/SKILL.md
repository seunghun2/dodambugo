...
- **대기 중인 태스크 (검수 완료 후 진행)**:
  - [ ] **솔라피 알림톡 템플릿 버튼 URL 검수 완료 시 코드 업데이트 (2026-08-09 승인 신청)**: 
    - 대표님이 솔라피에서 알림톡 버튼 URL(`https://bugoon.maeumbugo.co.kr/view/#{부고번호}?token=#{owner_token}&m=#{m}`) 수정 검수 신청 완료.
    - 카카오 승인 완료 후, 대표님이 말씀해 주시면 발급된 `templateId` 매칭 및 링크 실발송 테스트 진행.
  - [ ] **부고온 알림톡 10개 템플릿 검수 완료 시 적용**: 등록한 부고온 알림톡 템플릿 검수 승인 시 코드 매칭 및 실발송 테스트 진행.
- **관련 소스코드**:
