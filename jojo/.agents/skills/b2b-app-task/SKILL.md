...
### 4-2. B2B(부고온) 장지 만족도 조사 알림톡 발송 중단 및 교체 예정 (2026-09-17)
- **현황**: 매일 17시 발송되는 장지 이용후기 크론(`app/api/cron/burial-review-notify/route.ts`)에서 **B2B(부고온) 부고장은 발송 대상에서 전면 제외** (`.is('b2b_user_id', null)` 필터링 및 2중 가드 처리).
- **중단 사유**: 기존 솔라피 알림톡 템플릿(`KA01TP260714225958774NSjzKCXWvdF`) 본문에 "마음부고" 명칭이 노출되어 있어, B2B 제휴사/장례지도사의 고객(상주)에게 발송 시 브랜드 혼선 및 컴플레인 발생 위험.
- **향후 교체 계획 (TODO)**:
  - 현재는 B2C(순수 마음부고) 부고장에 대해서만 장지 후기 수집 알림톡 발송 유지.
  - 추후 "부고온" 또는 제휴 상조사 전용 브랜드 명의의 신규 알림톡 템플릿을 심사·승인받은 후 B2B용으로 교체 재개 예정.
...
### 13-23. 상조회사 본사용 어드민(/b2b/company) 실사용자 E2E 점검 및 화환+부의금 2대 정산서 완결 (2026-08-29)
- **점검 및 개선 내용**:
  1. **본사 계정 발급 무결성 보장**: `app/api/b2b/admin/companies/accounts/route.ts`에 `my_referral_code` 자동 생성 부여 로직을 추가하여 DB NOT-NULL 제약조건 위반 없이 즉시 계정 발급 완결.
  2. **로그인 세션 권한 연동**: `app/api/b2b/login/route.ts` 응답 `user` 객체에 `company_id` 필드를 반환하여, 상조회사 로그인 시 `/b2b/company/dashboard`로 0.1초 즉시 진입 완결.
  3. **데이터 철벽 격리(Data Isolation) 검증**: 상조회사 관리자 로그인 시, 소속 지도사(김미연)의 실적(화환 20,000원 + 부의금 1,650원)만 정확히 집계되고 타사 및 개인지도사 데이터는 100% 차단됨을 DB 및 Playwright 브라우저로 실데이터 검증 완료.
...
### 14. 더좋은라이프 제휴위탁운영 계약서 작성 중 (2026-09-18 업데이트)
- **상태**: 📝 작성 중 (검증 완료, 서명 전)
- **파일 위치**:
...
- **최근 수정 내역 (2026-09-18)**:
  - python-docx OOXML 정밀 생성기로 교체 (글꼴 10pt 통일, 표 깨짐 및 서명란 레이아웃 교정)
  - 부속서 제1조: '소비자 판매 기준가' 컬럼 삭제, 정산 수수료 중심 4열 표로 재편
  - 본문 제5조 제4항: '본사 귀속 수수료 일체 부가가치세(VAT) 포함' 명시 (화환, 부의금 3.3%, 답례품 최대 4% 일괄 적용)
