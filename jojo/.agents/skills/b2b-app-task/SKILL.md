...
- **설명**: 
  ...
  - `판매정보` (부고장에 부착될 화환 및 근조 상품 판매 정보 노출 여부 토글)
  - 위 3가지 옵션은 현재 백엔드 DB 및 부고장 뷰와 연동되지 않은 프론트엔드 UI 상태이므로 대표님 지시에 따라 설정 화면에서 **미노출 처리** 완료함.
  - 추후 파트너 요구사항 확정 시 DB 스키마(`b2b_users` 또는 `b2b_settings`) 추가 및 백엔드 API 연동 고도화 예정.
- **관련 소스코드**:
  - jojo/app/b2b/settings/page.tsx (설정 페이지)
  - jojo/app/b2b/view/[id]/ViewContent.tsx (부고장 뷰)
