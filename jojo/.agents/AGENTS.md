...
### 9. 과도한 코드 수정 금지 (절대 규칙!)
- 대표님이 요청하신 작업만 가장 **심플하고 직관적이며 안전하게** 수정할 것.
- 멋대로 복잡한 인코딩, 오버 엔지니어링, 불필요한 서버 설정(`export const dynamic` 등)을 덧붙여 사이드이펙트나 500 에러를 유발하는 행위 절대 금지.
- **최소한의 코드 변경**으로 요청받은 목적만 정확하고 깔끔하게 달성할 것.

### 10. Vercel 단일 배포 및 빌드 비용 절감 (필수 규칙!)
- `git push origin main`만으로 Vercel 프로덕션 배포가 자동 1회 실행됨.
- **수동 Deploy Hook(curl) 중복 호출 절대 금지!** (빌드 CPU 시간 낭비 및 추가 과금 원천 차단)
- 개발 도중 잦은 커밋/배포 절대 금지 → 로컬에서 `npm run build`로 100% 검증 후, 최종 확인 시에만 **단 1회 커밋 & 푸시**할 것.
