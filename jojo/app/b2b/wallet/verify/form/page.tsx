...
export default function VerifyFormPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                // 성공 알림 후 wallet 페이지로 리다이렉트
                // ( wallet 페이지 진입 시 user 정보 갱신을 강제하기 위해 로컬스토리지에도 임시로 셋팅 가능 )
                const b2bUser = localStorage.getItem('b2b_user');
                if (b2bUser) {
    ...
    return (
        <div className={styles.page}>
            ...
            <div className={styles.content}>
                <h1 className={styles.title}>개인 정보 입력(소득증빙)</h1>
                <p className={styles.redAlert}>
                    입력해주신 개인정보를 바탕으로 사업소득 신고가 진행될 예정입니다.
                    따라서 신청자와 예금주의 정보를 동일하게 입력 부탁드립니다.
                </p>
