...
interface FuneralHomeClientProps {
...
export default function FuneralHomeClient({ initialData }: FuneralHomeClientProps) {
    const router = useRouter();
    const [facilities, setFacilities] = useState<Facility[]>(initialData);
    const [filteredFacilities, setFilteredFacilities] = useState<Facility[]>(initialData);
    ...
    // 전체 데이터 백그라운드 로드 (initialData가 일부일 수 있으므로)
    useEffect(() => {
        const fetchAll = async () => {
            try {
                const allRes = await fetch('/api/facilities?all=true');
                const allJson = await allRes.json();
                if (allJson.data && allJson.data.length > facilities.length) {
                    setFacilities(allJson.data);
                    setFilteredFacilities(allJson.data);
                }
            } catch (err) {
                console.error('장례식장 전체 로드 실패:', err);
            }
        };
        fetchAll();
    }, []);
