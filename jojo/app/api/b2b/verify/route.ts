...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // 필수 필드 유효성 검사
        if (!identity_name || !rrn_front || !rrn_back || !identity_type || !identity_phone) {
        ...
        if (error) {
            console.error('본인인증 정보 저장 실패:', error);
            return NextResponse.json({ error: '본인인증 정보 저장에 실패했습니다.' }, { status: 500 });
        }

        return NextResponse.json({ success: true, message: '본인인증 및 소득증빙 등록이 완료되었습니다.' });
    } catch (err) {
