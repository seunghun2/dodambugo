...
- **용도**: 방문자 분석 + 이벤트 트래킹
- **Measurement ID (개발)**: G-6H5TT2F5RB
- **Measurement ID (프로덕션)**: G-41LNKTBH4W
- **GA4 콘솔**: https://analytics.google.com (마음부고 속성 선택)
- **컴포넌트**: `components/GoogleAnalytics.tsx`

#### 트래킹 이벤트 목록

| 이벤트명 | 설명 | 트리거 위치 |
|---------|------|-----------|
| `오버레이_노출` | 발인 후 오버레이 표시 | `ViewContent.tsx` |
| `오버레이_장지비교클릭` | "장지 비교하기" 버튼 클릭 | `ViewContent.tsx` |
| `오버레이_부의금클릭` | "부의금 전하기" 버튼 클릭 | `ViewContent.tsx` |
| `폼_신청자정보` | 폼 신청자 섹션 도달 | `create/[templateId]/page.tsx` |
| `폼_장례식장정보` | 폼 장례식장 섹션 도달 | `create/[templateId]/page.tsx` |
| `폼_부고정보` | 폼 부고정보 섹션 도달 | `create/[templateId]/page.tsx` |
| `폼_추가상주` | 폼 추가상주 섹션 도달 | `create/[templateId]/page.tsx` |
| `폼_발인일시` | 폼 발인일시 섹션 도달 | `create/[templateId]/page.tsx` |
| `폼_안내사항` | 폼 안내사항 섹션 도달 | `create/[templateId]/page.tsx` |
| `폼_제출시도` | 제출 버튼 클릭 | `create/[templateId]/page.tsx` |
| `폼_유효성실패` | 유효성 검사 실패 (실패 필드 포함) | `create/[templateId]/page.tsx` |
| `폼_임시저장` | 임시저장 후 홈으로 이동 | `create/[templateId]/page.tsx` |

#### GA4 API 조회 (서비스 계정)

- **GCP 프로젝트**: daedaesonson (`gen-lang-client-0924143877`)
- **서비스 계정**: `sheets-writer@gen-lang-client-0924143877.iam.gserviceaccount.com`
- **키 파일**: `/Users/seunghun/Desktop/dodam/service_account.json`
- **GA4 Property ID**: `518592689`
- **필요 API**: Google Analytics Data API, Google Analytics Admin API (GCP 콘솔에서 활성화 필요)
- **GA4 속성 액세스**: GA4 콘솔 → 관리 → 속성 액세스 관리 → 서비스 계정 이메일을 뷰어로 추가
...
- **컴포넌트**: `components/MicrosoftClarity.tsx`
