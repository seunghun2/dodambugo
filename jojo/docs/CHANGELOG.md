...
### B2B 부고장 제작 로고 선택 UI 및 특정 로고 개선 (피드백 반영)
...
- **로고 이미지 내부 캡처된 별표(즐겨찾기) 아이콘 삭제**:
  - 로고 원본 스크린샷 캡처 시 카드 우측 상단에 구워져 포함되어 있었던 별 모양 아이콘 영역을 흰색으로 완전히 지운(Painting-over) 뒤 Bounding Box를 계산하여 크롭하도록 파이썬 스크립트(`process_logos.py`, `fix_thejoeun_logo.py`)를 개선했습니다.
  - 이로써 모든 상조사 로고 이미지 자체에서 노이즈인 별표가 완벽하게 배제되었습니다.
