...
### 📱 B2B 부고장 제작 화면 모바일 가상 키패드 가림 방지 (UX 개선)
- **파일**: `app/b2b/create/page.tsx`, `app/b2b/create/create.module.css`
- 모바일(iOS/Android) 환경에서 상주 성함, 연락처, 호실, 장지 등 인풋 필드 터치(포커스) 시 가상 키패드가 입력창을 가리지 않고 화면 중앙으로 부드럽게 스크롤되도록 `onFocusCapture` 기반 `scrollIntoView` 자동 스크롤(300ms 딜레이) 구현.
- 화면 최하단 상주 추가 및 옵션 입력 시 키패드 뒤로 가려지는 현상을 차단하기 위해 페이지 하단 스크롤 여유 공간(`padding-bottom: 240px`) 확보.

### ✉️ 부고 문자 및 카카오톡 공유 템플릿 줄바꿈 & 조사('에\n') 분리 버그 수정
- **파일**: `app/b2b/view/[id]/ViewContent.tsx`, `app/view/[id]/ViewContent.tsx`, `app/create/complete/[bugoNumber]/page.tsx`
- 별세일시가 없거나 서식이 다를 때 발생하던 `故 OOO 님께서 (상주 OOO)\n에\n별세하셨기에...` 어색한 조사 분리 및 줄바꿈 버그 근본 해결.
- `故 OOO 님께서 2026년 9월 2일 별세하셨기에 부고를 전해드립니다.\n(상주: OOO)` 형태로 품격 있고 자연스러운 표준 부고 문구로 개선.

### 🌸 B2B 화환 주문서 전면 개편 및 실시간 지역/도서산간 할증 & 배송제외 시스템 구축
- **파일**: `app/b2b/flower/order/[productId]/OrderContent.tsx`, `app/b2b/flower/order/[productId]/order.module.css`, `app/b2b/flower/page.tsx`
- **주문 폼 직관화**: 주문자 정보 및 고인 성함 입력 필드 제거 ➡️ [받으시는 상주 성함 + 필수 연락처] 체계로 단일화.
- **연락처 UX 강화**: 숫자 전용 키패드(`inputMode="numeric"`), 자동 하이픈(`010-XXXX-XXXX`), 11자리 제한 적용.
- **장례식장/주소 검색 모달 연동**: 주소 검색 팝업에서 도로명 주소 선택 시 주소창 및 장례식장명 자동 매핑, 탭 전환 무한루프 버그 수정.
- **실시간 지역/산간 할증 시스템**: 전국 화원 ERP 전수 조사 데이터 기반, 주소/장례식장 입력 즉시 권역별(+10,000원) 및 도서산간 특수지역(+10,000~20,000원) 할증 실시간 자동 계산 및 결제 금액 반영.
- **배송 불가 지역 차단**: 울릉, 백령, 연평 등 화원 직배송 불가지역 사전 감지 및 주문 접수 차단 (역마진 원천 방어).
- **디자인 개선**: 과도한 원색/이모지 제거, 세련된 뉴트럴 톤 안내 박스 및 가격 `~` 표기 통일.

### 🗂️ 화환 임시 부고 관리 목록 격리 필터링 & 계층적 뒤로가기 흐름
- **파일**: `app/b2b/manage/page.tsx`, `components/GlobalBackButtonHandler.tsx`
- B2B 화환 단독 발주 시 생성되는 가상/임시 부고(`template_id: 'flower_only'`)가 파트너의 실제 장례 부고 관리 목록에 섞여 나오지 않도록 완전 격리 필터링.
- 화환 결제창 ➡️ 주문서 ➡️ 화환 목록 ➡️ 마이페이지 ➡️ 대시보드로 이어지는 계층적 뒤로가기 라우팅 완성.

### ⚡ Next.js 15+ Turbopack 개발 에러 오버레이 해결
- **파일**: `app/layout.tsx`
- Mantine `ColorSchemeScript` 경량화 및 불필요한 개발 스크립트 오버레이 제거로 개발 환경 안정화.

### 📊 더좋은라이프 미팅 녹취록 분석 & 상조사 다단계 수당/마진 정밀 역산
- **파일**: `더좋은라이프 미팅/새로운 노트 (3, 4).txt`
- 화원 ERP 74장 전수 조사 데이터(기본 3.3만, 고급 4.3만, 특대 5.3만) 기반, 기본형(109,000원) 시 지도사 50,000원 + 상조 본사 10,000원 + 플랫폼 마진 12,400~15,700원 정밀 역산.
- 외부 파트너 추천 시 [추천인 3,500원 + 상조 본사 6,500원] 귀속 로직 안전성 검증 (플랫폼 추가 비용 0원, 마진 100% 보존).
...
### 🪪 파트너 신분증 뷰어 관리자 전용 가드 적용
- **파일**: `app/api/b2b/admin/id-card-view/route.ts`
- 관리자 로그인(`verifyAdmin`)이 확인된 경우에만 임시 서명 URL 생성을 허용하여 외부 신분증 사진 유출 원천 차단.

### 📋 파트너 본인인증 API 정돈 (모의 OCR 제거 및 실명 대조 저장)
...
### 📱 부고장 뷰어(ViewContent) 공유 링크 및 상주 꼬리표(?m=) 보존 보강
- **파일**: `app/view/[id]/ViewContent.tsx`
- 부고장 화면 진입 시 일회용 토큰 청소 과정에서 `?m=` 파라미터가 유실되지 않도록 보존 처리.
- 화면 내 [공유하기] 및 [링크 복사] 클릭 시 접속한 도메인(`bugoon.maeumbugo.co.kr` / `maeumbugo.co.kr`) 및 상주 꼬리표(`?m=`)가 유지된 채로 정확하게 복사/공유되도록 보정.

### 🔍 조의금 및 답례품 페이지 UUID / bugo_number 듀얼 조회 완벽 지원
- **파일**: `app/view/[id]/condolence/page.tsx`, `app/view/[id]/gift/page.tsx`
- 4자리 단축 번호뿐만 아니라 긴 고유 식별자(UUID) 링크로 접속하더라도 부고 및 계좌 정보를 100% 정상 조회하도록 호환성 보강.
- 답례품 페이지의 DB 테이블명 오타(`bugos` ➡️ `bugo`) 수정.
