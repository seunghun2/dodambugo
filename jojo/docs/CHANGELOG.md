...
- **개발자 도구 (F12) 보안 차단**: 프로덕션 환경에서 보안 및 크롤링 방지를 위해 개발자 도구(F12, Cmd+Opt+I 등 단축키) 작동을 감지하면 아예 주소창을 `about:blank`(빈 페이지)로 강제 리다이렉트 처리하여 기존 사이트 정보와 소스코드를 완전히 비워버리도록 하였습니다. (단, 복사/붙여넣기 사용을 위해 마우스 우클릭은 정상적으로 허용하며, 로컬 개발 환경인 `localhost` 및 `127.0.0.1`은 제외)
