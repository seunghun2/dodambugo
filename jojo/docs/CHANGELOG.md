...
- **개발자 도구 (F12) 및 우클릭 보안 차단**: 프로덕션 환경에서 보안 및 크롤링 방지를 위해 개발자 도구(F12, Cmd+Opt+I 등 단축키) 및 마우스 우클릭(contextmenu) 작동을 전면 제한하고, 감지 시 화면 전체를 검은색 오버레이 화면으로 전환 차단하도록 하였습니다. (단, 로컬 개발 환경인 `localhost` 및 `127.0.0.1`은 제외)
